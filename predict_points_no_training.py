...
def main():
    ...
    # Comprova si el model ja està guardat
    if os.path.exists(model_path):
        ...
        print("\nEntrenant el model...")
        model, _, _ = train_model(df, feature_columns, target)

        # Crea la carpeta si no existeix i guarda el model
        os.makedirs('model', exist_ok=True)
        print("\nGuardant el model...")
        joblib.dump(model, model_path)

    # Carrega les dades per prediccions
    df, feature_columns, _ = prepare_data('data/fantasy_data.csv')

    # Mostrar jugadors i equips disponibles
    ...
    # Demanar inputs de l'usuari per fer prediccions
    while True:
